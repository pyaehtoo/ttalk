...
class SentHistoryPage extends StatefulWidget {
  const SentHistoryPage({Key? key}) : super(key: key);

  @override
  State<SentHistoryPage> createState() => _SentHistoryPageState();
}

class _SentHistoryPageState extends State<SentHistoryPage> {
  ...
  void filterList(String imsAccount) async {
    setState(() {
      filteredTransactionList = transactionList
          .where((trasaction) => trasaction.accountFrom == imsAccount)
          .toList();
    });
  }
                            ...
                            child: SizedBox(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  if (index == 0) {
                                    return const SentHistoryTableTitleSectionView();
                                  }
                                  TransactionVO transactionVO =
                                      filteredTransactionList![index - 1];
                                  return SentHistroyTransactionItem(
                                    transactionVO: transactionVO,
                                  );
                                },
                              ),
...
class SentHistoryTableTitleSectionView extends StatelessWidget {
  const SentHistoryTableTitleSectionView({
    super.key,
  });
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SentHistroyTransactionItem(transactionVO: null),
        const SizedBox(
          height: kMarginMedium2,
        ),
        Container(
...
class SentHistroyTransactionItem extends StatelessWidget {
  final TransactionVO? transactionVO;
  const SentHistroyTransactionItem({
    super.key,
    ...
    return Row(
      ...
      children: [
        ...
        SizedBox(
          width: 120,
          child: Text(
            transactionVO?.toUser?.userName ?? 'Sent To',
            style: kTextStyle,
            textAlign: TextAlign.center,
          ),
        ),
        ...
        SizedBox(
          width: 120,
          child: Text(
            transactionVO?.iceAmount?.toString() ?? 'Used ICE',
            style: kTextStyle,
            textAlign: TextAlign.center,
          ),
        ),
